...
def run_clustering(data, plot):
    ...
    agg_avg = run_cluster_func_ntimes(data, agglomerate_clustering, plot_last=plot, num_times=3, data_size=800)
    print(f"Agg num cls avg:     {agg_avg} -> {round(agg_avg)}")
    cls_counts.append(round(agg_avg))

    chain_avg = run_cluster_func_ntimes(data, chain_clustering, plot_last=plot, num_times=100, data_size=300)
    print(f"Chain num cls avg:   {chain_avg} -> {round(chain_avg)}")
    cls_counts.append(round(chain_avg))

    maximin_avg = run_cluster_func_ntimes(data, maximin_clustering, plot_last=plot, num_times=100, data_size=300)
...
def run_cluster_func_ntimes(data, func, plot_last, num_times, data_size=300):
    num_cls_list = []
